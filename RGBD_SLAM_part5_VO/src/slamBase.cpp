...
Eigen::Isometry3d cvMat2Eigen(Mat &revc, Mat&tvec){
    //本函数具体的说明在上一次的4中(点云拼接)有具体说明.
    cv::Mat R;
    cv::Rodrigues( revc, R );
    Eigen::Matrix3d r;
    cv::cv2eigen(R, r);
    /*
    如果出现在cv里找不到eigen的报错,可能是函数头引入的顺序问题?我也不知道为什么,反正调换顺序后我就编译成功了
    当然,我们也可以不用这个函数,用下面这样写法:
    for ( int i=0; i<3; i++ )
        for ( int j=0; j<3; j++ ) 
...
PointCloud:: Ptr joinPointCloud(PointCloud::Ptr original, 
    ...
    *newCloud += *output ;//+=操作也是对应点云，点云指针不可以(另外这里的加法顺序好像没有规定)

    //Voxel grid滤波降采样
    static pcl::VoxelGrid<PointT> voxel;
    double gridsize = 0.02; //点云的分辨率
